...
class MTDNNModel(object):
    ...
    def _setup_optim(self, optimizer_parameters, state_dict=None, num_train_step=-1):
        ...
        if self.config['optimizer'] == 'AdamW':
            print("quiOPT")
            self.optimizer = optim.AdamW(optimizer_parameters, self.config['learning_rate'],
                                       weight_decay=self.config['weight_decay'])

        #ho commentato warmup
    ...
    def update(self, batch_meta, batch_data):
        ...
        if self.config.get('mkd_opt', 0) > 0 and ('soft_label' in batch_meta):
            labels=batch_data[batch_meta['label']]###aggiunto anche la label
            ...
            kd_loss = kd_lc(logits, soft_labels,labels, int(self.epoch)/10, weight, ignore_index=-1) if kd_lc else 0
    ...
    def predict(self, batch_meta, batch_data):
        ...
        #logiti=score
        ###Se voglio fare smoothing delle predizioni del teacher
        #divido i logits per la temperatura
        #score=score/4
        if task_obj is not None:
            ...
            return logiti,scores, predictions, batch_meta['answer']
        else:
    ...
    def load(self, checkpoint):
        ...
        if 'state' in model_state_dict:
            ...
            self.network.load_state_dict(sd['state'], strict=False)
            
        #HO modificato recentemente questa parte per provare il continuos Multitask learning
       #devo commentare optimizer altrimenti non si trova col numero dei ask
        if 'optimizer' in model_state_dict:
            
            self.optimizer.load_state_dict(model_state_dict['optimizer'])
         #Pure qua ho modificato il config   
