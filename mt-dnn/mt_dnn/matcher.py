...
class SANBertNetwork(nn.Module):
    def __init__(self, opt, bert_config=None, initial_from_local=False):
        ...
        if not initial_from_local:
            ...
            #ModificaMIA self.bert = model_class.from_pretrained(opt['init_checkpoint'])
            #sotto ci sono le righe aggiunte da me
            #print("AAAAAAA")
            #self.preloaded_config=AutoConfig.from_pretrained(opt['init_checkpoint'])
            self.bert = model_class.from_pretrained(opt['init_checkpoint'],config=self.preloaded_config )
        else:
           # self.preloaded_config = config_class.from_dict(opt)  # load config from opt
            self.preloaded_config=config_class.from_pretrained(opt['init_checkpoint'])
